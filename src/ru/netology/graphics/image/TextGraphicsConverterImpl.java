...
public class TextGraphicsConverterImpl implements TextGraphicsConverter {
    ...
    public String convert(String url) throws IOException, BadImageSizeException {
        ...
        if (newWidth > maxWidth || newHeight > maxHeight) {
            Double widthRate = (double) newWidth / maxWidth;
            Double heightRate = (double) newHeight / maxHeight;
            double rate = (widthRate.compareTo(heightRate) > 0) ? widthRate : heightRate;
            newWidth = (int) Math.round(newWidth / rate);
            newHeight = (int) Math.round(newHeight / rate);
        }
        ...
        char[][] buffer = new char[newHeight][newWidth];
        ...
        // от схемы.
        return printPixels(buffer);
    }

    private static String printPixels(char[][] buffer) {
        StringBuilder out = new StringBuilder();
        for (int w = 0; w < buffer.length; w++) {
            for (int h = 0; h < buffer[w].length; h++) {
                out.append(String.format("%s%s", buffer[w][h], buffer[w][h]));
                if (h == buffer[w].length - 1) out.append("\n");
            }
        }
        return out.toString();
    }
